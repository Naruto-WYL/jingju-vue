...
function fitViewBox(svg, nodes) {
  ...
  let minX = d3.min(nodes, (node) => node.x - Math.max(node.r + 28, node.plateWidth / 2 + 18)) ?? 0
  let maxX = d3.max(nodes, (node) => node.x + Math.max(node.r + 28, node.plateWidth / 2 + 18)) ?? VIEW_WIDTH
  let minY = d3.min(nodes, (node) => node.y - node.r - 22) ?? 0
  let maxY = d3.max(nodes, (node) => node.y + node.r + (node.core ? 64 : 56)) ?? VIEW_HEIGHT
...
function drawAvatarClips(defs, nodes) {
  const clips = defs
    .selectAll('clipPath')
    .data(nodes)
    .join('clipPath')
    .attr('id', (node) => `${node.uid}-clip`)
...
function layoutGraph(nodes, links, nodeById) {
  ...
  assigned.forEach((groupNodes, coreId) => {
    ...
    groupNodes
      ...
      .forEach((node, index) => {
        const t = groupNodes.length === 1 ? 0.5 : index / (groupNodes.length - 1)
        const angle = spread.start + (spread.end - spread.start) * t
        const radius = 210 + (index % 3) * 34 + Math.min(36, node.degree * 4)
        const point = polarPoint(core.anchorX, core.anchorY, radius, angle)
        node.anchorX = clamp(point.x, 82, VIEW_WIDTH - 82)
        node.anchorY = clamp(point.y, 84, VIEW_HEIGHT - 94)
        node.x = node.anchorX
        node.y = node.anchorY
      })
  })
  ...
  unanchored.forEach((node, index) => {
    ...
    const point = polarPoint(VIEW_WIDTH / 2, VIEW_HEIGHT / 2, 260, angle)
    node.anchorX = clamp(point.x, 82, VIEW_WIDTH - 82)
    node.anchorY = clamp(point.y, 84, VIEW_HEIGHT - 94)
    node.x = node.anchorX
    node.y = node.anchorY
  ...
  const simulation = d3
    ...
    .force(
      ...
      d3
        ...
        .id((node) => node.id)
        .distance((link) => 155 - Math.min(55, link.weight * 8))
        .strength(0.28),
    )
    .force('charge', d3.forceManyBody().strength((node) => (node.core ? -520 : -260)))
    .force('x', d3.forceX((node) => node.anchorX).strength((node) => (node.core ? 0.72 : 0.36)))
    .force('y', d3.forceY((node) => node.anchorY).strength((node) => (node.core ? 0.72 : 0.36)))
    .force('collision', d3.forceCollide((node) => node.r + 52).strength(0.9))
    .stop()
  ...
  nodes.forEach((node) => {
    node.x = clamp(node.x, node.r + 34, VIEW_WIDTH - node.r - 34)
    node.y = clamp(node.y, node.r + 42, VIEW_HEIGHT - node.r - 70)
  })
}
...
function getCorePositions(count) {
  ...
  if (count === 2) {
    return [
      { x: VIEW_WIDTH * 0.38, y: VIEW_HEIGHT * 0.46 },
      { x: VIEW_WIDTH * 0.66, y: VIEW_HEIGHT * 0.43 },
    ]
  }

  return [
    { x: VIEW_WIDTH * 0.42, y: VIEW_HEIGHT * 0.47 },
    { x: VIEW_WIDTH * 0.56, y: VIEW_HEIGHT * 0.24 },
    { x: VIEW_WIDTH * 0.74, y: VIEW_HEIGHT * 0.47 },
    { x: VIEW_WIDTH * 0.28, y: VIEW_HEIGHT * 0.52 },
    { x: VIEW_WIDTH * 0.56, y: VIEW_HEIGHT * 0.64 },
  ]
}
...
function drawEdges(edgeLayer, links, nodeById) {
  const edgeGroups = edgeLayer
    ...
    .on('mouseenter', (event, edge) => {
      setActive(edge.source, edge.target)
      showEdgeTooltip(event, edge, nodeById)
    })
    .on('mousemove', (event, edge) => showEdgeTooltip(event, edge, nodeById))
    .on('mouseleave', () => {
      clearActive()
      hideTooltip()
    })
  ...
  edgeGroups
    ...
    .attr('stroke-width', (edge) => Math.min(5.2, 2.4 + edge.weight * 0.46))
...
function drawNodes(nodeLayer, nodes, links, nodeById) {
  const nodeGroups = nodeLayer
    ...
    .on('mousemove', (event, node) => showNodeTooltip(event, node, links, nodeById))
    .on('mouseleave', () => {
      clearActive()
      hideTooltip()
    })
  ...
  nodeGroups
    ...
    .attr('clip-path', (node) => `url(#${node.uid}-clip)`)
  ...
  const plaques = nodeGroups
    ...
    .attr('transform', (node) => `translate(0,${node.r + 6})`)
...
function edgePath(edge, nodeById, index) {
  ...
  const bendSize = Math.min(86, 28 + distance * 0.1)
...
function setActive(sourceId, targetId = '') {
  const connected = new Set([sourceId, targetId].filter(Boolean))

  currentGraph.value.links.forEach((edge) => {
    if (edge.source === sourceId || edge.target === sourceId || edge.source === targetId || edge.target === targetId) {
  ...
  svg
    .selectAll('.edge')
    .classed('is-muted', (edge) => !(connected.has(edge.source) && connected.has(edge.target)))
    .classed('is-active', (edge) => connected.has(edge.source) && connected.has(edge.target))
}
...
:deep(.node__plaque) {
  filter: drop-shadow(0 3px 2px rgba(70, 30, 12, 0.24));
}
